		...
		const Matx34d& P1)
{
	...
	ofstream fout("Cloud.txt");
	for(int i = 0; i < pointCloud.size(); i++) {
		fout<<pointCloud[i].x<<" "<<pointCloud[i].y<<" "<<pointCloud[i].z<<"\n";
	}
	fout.close();
...
int main()
{
	Mat right_frame_orignal = imread("LEFT1.jpg");
	Mat left_frame_orignal  = imread("LEFT2.jpg");
